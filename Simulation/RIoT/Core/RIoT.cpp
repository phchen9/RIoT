...
)
{
    ...
    if (DEVICE_ID_CERT_TYPE == RIOT_SELF_SIGNED) {
        ...
        status = RiotCrypt_Verify(derCtx.Buffer, derCtx.Position, &tbsSig, &eccRootPub);
        if (status != RIOT_SUCCESS)
        {
            fprintf(stderr, "RIOT: ERROR: signature of DeviceID certificate is invalid\n");
            return;
        }
    ...
    if (DEVICE_ID_CERT_TYPE == RIOT_ROOT_SIGNED)
    {
        ...
        if (status != RIOT_SUCCESS)
        {
            fprintf(stderr, "RIOT: ERROR: signature of root certificate is invalid\n");
            return;
        }
...
)
{
    ...
    uint8_t rootX[RIOT_COORDMAX] = {
        0x01, 0x01, 0x66, 0x5D, 0x9F, 0xEB, 0x08, 0x14, 0xA9, 0x1F, 0xFA, 0x69,
        0xAB, 0x7B, 0xEB, 0xDC, 0x23, 0x2A, 0x64, 0xC3, 0x18, 0xBD, 0xB2, 0x8F,
        0xB9, 0x00, 0xEE, 0xAC, 0x1C, 0x44, 0xCB, 0xE0, 0x5D, 0x84, 0x10, 0xE4,
        0xB4, 0x0F, 0x4D, 0xF9, 0x2E, 0xC6, 0xB5, 0xF0, 0xC2, 0x52, 0x39, 0xED,
        0x6A, 0x6B, 0xFF, 0xD4, 0xB6, 0xBB, 0x61, 0xA1, 0xF4, 0x06, 0x94, 0x1D,
        0xBC, 0xC1, 0xAD, 0x2F, 0x16, 0xFC
    };

    uint8_t rootY[RIOT_COORDMAX] = {
        0x00, 0xF0, 0xD4, 0x04, 0xBE, 0x99, 0xDF, 0x77, 0x50, 0x46, 0x65, 0xBD,
        0x2F, 0xD3, 0xC9, 0x46, 0xCD, 0x4B, 0x62, 0x0F, 0xC3, 0x28, 0x32, 0xCF,
        0x03, 0xD8, 0x1A, 0xF0, 0x34, 0xE7, 0x10, 0xF4, 0x6B, 0x42, 0x67, 0xBA,
        0xB8, 0x87, 0xD8, 0xE5, 0xE1, 0x35, 0x47, 0x3C, 0xA6, 0xBB, 0x19, 0x3D,
        0x44, 0x7D, 0xE0, 0x94, 0xB7, 0xC7, 0xE5, 0x2F, 0x2D, 0xF5, 0xB3, 0x06,
        0xA0, 0xC9, 0x94, 0xAE, 0x7A, 0x6F
    };

    uint8_t rootD[RIOT_COORDMAX] = {
        0x00, 0x6A, 0x82, 0x83, 0xC8, 0xD4, 0x45, 0xE8, 0x83, 0xE8, 0xCF, 0x2C,
        0x07, 0x55, 0x04, 0x64, 0x30, 0x24, 0x9B, 0x8D, 0x04, 0x67, 0x6B, 0xC7,
        0x6E, 0xB2, 0xC6, 0x14, 0x72, 0x71, 0x61, 0x16, 0x8B, 0xA4, 0x83, 0x1D,
        0x61, 0x45, 0xD2, 0xF6, 0x3E, 0xDB, 0xE1, 0x4D, 0xDD, 0xD0, 0xC3, 0x0D,
        0x1A, 0xD5, 0x67, 0x86, 0x0C, 0xB6, 0xD1, 0x4D, 0xD8, 0x97, 0x19, 0xB2,
        0xC2, 0x77, 0xC4, 0x88, 0xFC, 0x82
    };
